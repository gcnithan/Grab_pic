...
export default function OrganizerDashboard() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      try {
        ...
        const fetchedEvents = (data.events || []).map(ev => ({
          id: ev.id,
          name: ev.name,
          code: ev.join_code,
  ...
  const createEvent = async () => {
    ...
    try {
      ...
      // The API returns { event: { id: ..., name: ..., join_code: ... } }
      const newEv = {
        id: data.event.id,
        name: data.event.name,
        code: data.event.join_code,
        date: new Date(data.event.created_at).toLocaleDateString(),
        photos: 0
      };
